...
* This project can easily be deployed on platforms such as Render, Heroku, Replit, etc.
* On Render: set *Build Command* to `npm install`, *Start Command* to `node server.js`, and add the environment variables in the Render dashboard.
...
* **Sessions & store**: `express-session` configured with `key: 'express.sid'` and a `MongoStore` store for persistence.
* **Socket authentication**: `passport.socketio` is configured to parse session cookies and link socket connections to authenticated user sessions.
* **Real-time events**:

  * `'user'` event: emits `{ username, currentUsers, connected }` when users join or leave.
  * `'chat message'` event: clients emit with message text; server broadcasts `{ username, message }` to all clients.
* **Client listeners**:
